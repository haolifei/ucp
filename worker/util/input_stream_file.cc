...
baidu::ucp::system::ErrorCode InputStreamFile::GetLastError() {
    return ERRORCODE(errno_, "%s", strerror(errno_));
}

baidu::ucp::system::ErrorCode InputStreamFile::ReadLine(std::string& line) {
    assert(IsOpen());
    size_t size;
    char* buf = NULL;
...
baidu::ucp::system::ErrorCode InputStreamFile::Read(void* buf, size_t& size) {
    assert(IsOpen());
    assert(NULL != buf);
    assert(size > 0);
